...
export const metadata: Metadata = {
  ...
  description: 'Shop contact lenses with free delivery across India.',
}
...
// ST-004 (A4. Contact Lenses Catalog). Search/sort/pagination reuse the same
// pattern as /shop, scoped to category='contacts'. The ticket's "power
// selector validates SPH/CYL/axis" AC is not implemented — that's a
// per-purchase power selection at add-to-cart time, not a catalog-browsing
// concern, and needs its own product-variant/cart data model. This ships the
// real catalog; the power selector is a stated gap, not silently dropped.
export default async function ContactsPage({ searchParams }: ContactsPageProps) {
  ...
  const { products, totalCount } = await getCatalogProductsByCategory('contacts', {
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="mb-8">
        <h1 className="text-dark">Contact Lenses</h1>
